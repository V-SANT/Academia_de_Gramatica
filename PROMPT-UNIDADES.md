...
E abre `src/data/unidades/u41-45.js` para veres o estilo à letra. Segue-o.
...
`pdf{2U+4}.png` (exercícios). Unidade 46 → `pdf095.png` e `pdf096.png`.
...
2. `python scripts/ver-chave.py fontes/chave.txt 46` para as respostas.
...
`src/data/unidades/u46-50.js`, que exporta `UNIDADES_46_50` e é reexportado por
...
const units = {};
for (let n = 1; n <= 50; n++) units[n] = { seed: { results: { 1: muitas }, answers: {} } };
...
As Unidades 46 a 50 transcritas, ligadas, verificadas contra a chave com zero
