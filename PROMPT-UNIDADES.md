...
"Gramática Ativa 1" (Lidel). As Unidades 1 a 35 já estão feitas e verificadas.
Preciso que transcrevas as **Unidades 36 a 40** seguindo exactamente o mesmo
...
E abre `src/data/unidades/u31-35.js` para veres o estilo à letra. Segue-o.
...
`pdf{2U+4}.png` (exercícios). Unidade 36 → `pdf075.png` e `pdf076.png`.
...
2. `python scripts/ver-chave.py fontes/chave.txt 36` para as respostas.
...
`src/data/unidades/u36-40.js`, que exporta `UNIDADES_36_40` e é reexportado por
...
const units = {};
for (let n = 1; n <= 40; n++) units[n] = { seed: { results: { 1: muitas }, answers: {} } };
...
As Unidades 36 a 40 transcritas, ligadas, verificadas contra a chave com zero
